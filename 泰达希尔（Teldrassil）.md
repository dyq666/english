## The Balance of Nature （我）自然的平衡 （官）自然的平衡
...
Vicious little grell! 

可恶的小劣魔！

They`'ve been stealing` from our foodstores again. 

他们`一直从`我们食品存储处`偷`东西。

If you `wouldn't mind` `lending` a hand, they `are` sure `to have` my `stolen` bags within their camps to the west. （doing 作宾语，to do 用在主系表结构之后，done 作定语）

如果`你不介意`帮个忙，他们一定`把`从我这`偷走的`包放到西边的营地中了。

I `would be` glad `to reward` you if you `can bring` them back to me.

我很高兴`奖励`你，如果你能`把`它们`带`回给我。（to do 用在主系表结构之后）

### Progress

I `don't have` any spare supplies for my customers until my `stolen` goods are recovered.

在我被偷的物品被找回前，我`没有`多余的供给品给我的访客了。

### Completion

Elune `be` praised! And you as well, young \<class\>.

赞美艾露恩！也赞美你，年轻的 \<class\>。

There `are` many young night elves in need of these, but I'd be more than happy to give you one of the bags as a reward. 

虽然有许多需要那些供给品的年轻德鲁伊，但是我很高兴给你一个作为奖励。

I`'m` sure it `will come` in handy.（从句用在主系表结构之后）
...
Our friend Iverron `has fallen` to poison from the nearby webwood spiders. 

我们的朋友埃沃隆`中了`附近树林蜘蛛的毒。

I `can tend to` him `to keep` his state from deteriorating, but I `need` someone to gather an ingredient `to prepare` him an antidote.（to do 作目的状语，to do 作宾语，to do 作目的状语）

我会照顾他，使他的状态避免恶化，但我需要一个人去收集材料，以便为他准备解毒剂。

All around the ponds you `should find` Moonpetal Lilies. 

你`应该可以找到`月牙百合花，在水池周围。

These flowers `have` great healing properties, and I `can use` them with the ingredients I already have `to restore` Iverron to better health.（从句作定语，to do 作宾语）

这些花`有`强大的治疗能力，所以我`能用`他们和我已经有的材料一起，`恢复` Iverron 的健康。

Would you `gather` them for me? 

你能帮我`收集`一些吗？

There `should be` many just in sight of here, `near` the water's edge.（感觉是同位语）

仅仅在这里的视野中`就应该有`许多月牙百合花，`在`水的边缘`附近`。

### Progress

Our dear friend `is not` doing well, even with my mending. 

我们亲爱的朋友`的状态不是`很好，即使有我的治疗。

`Have` you `brought` the supplies I require?
...
这些事`将`完美地`进行`（这里 These 应该指的是制作解毒剂的事）。（官方翻译：这些就很不错了。）

Thank you, \<name\>. Your swiftness `is` appreciated.
...
I`'d hoped` `to never return to` Teldrassil for such grim business. （to do 作宾语）
...
It `seems` a greater threat `looms`.（从句作宾语）
...
| 物品     | Splintered Spider Fang  | 蜘蛛尖牙碎片     | 断裂的蜘蛛牙     |
