...
在[一般用法](#一般用法)的基础上补充一些信息, 通常译为: 在...之前的\$\{时间名词短语\}.
...
- [出处](https://www.ldoceonline.com/dictionary/as-soon-as)

    I came `as soon as` I heard the news.

    译为: `在`我听到消息`之后不久`, 我进来了.
...
英文解释: from a particular time or event in the past until the present, or in that period of time

译为: 在...之后直到现在. 实际上 "since" 和 "after" 都代表主句在从句之后发生, 只不过 "since" 多了一个持续性.

此外, 还有 "ever since" 的用法, 这里 ever 修饰 since, 译为: 在...之后直到现在`的任何时间`, 目前我没有明白 "ever since" 和 "since" 的区别, 可能是 "ever since" 程度更强一些.
...
- [conjunction]
    ...
    译为: `在`我离开大学`之后直到现在`, 我没有打过橄榄球.

- [preposition]

    We've been waiting here ` since` two o'clock.

    译为: `在`两点`之后直到现在`, 我们一直在这里等.

- [adverb]

    She left London ten years ago, and I haven't seen her `since`.

    译为: 她在十年前离开了伦敦, `在那时之后直到现在`, 我没在见过她.

- [ever since]

    We've been friends `ever since` we were at school together.

    译为: `在`我们一起上学`之后直到现在的任何时间`, 我们一直是朋友.

## while, during, throughout

### 概述

这些词都译为 "在...期间" 或 "在...整个期间" 或二者都有. 当谓语动词是短暂性时, 译为 "在...期间", 当谓语动词是持续性时, 译为 "在...整个期间".

|            | 在...期间 | 在...整个期间 | conjunction | preposition | adverb |
| ---------- | --------- | ------------- | ----------- | ----------- | ------ |
| while      | ✅         | ✅             | ✅           |             |        |
| during     | ✅         | ✅             |             | ✅           |        |
| throughout |           | ✅             |             | ✅           | ✅      |
...
- [while, 短暂性]  [出处](https://www.ldoceonline.com/dictionary/while)

    They arrived `while` we were having dinner.

    译为: `在`我们吃晚饭`期间`, 他们到了.

- [while, 持续性]  [出处](https://www.ldoceonline.com/dictionary/while)

    Would you look after the children `while` I do the shopping?

    译为: `在`我购物`整个期间`, 你能照顾下孩子们吗?

- [during, 短暂性]  [出处](https://www.ldoceonline.com/dictionary/during)

    My father was killed `during` the war.

    译为: `在`战争`期间`, 我父亲被杀了.

- [during, 持续性]  [出处](https://www.ldoceonline.com/dictionary/during)

    Foxes remain hidden `during` the day.

    译为: `在`白天`整个期间`, 狐狸都保持隐蔽.

- [throughout, preposition]  [出处](https://www.ldoceonline.com/dictionary/throughout)

    We are open every weekend `throughout` the year.

    译为: `在`这年`整个期间`, 我们每周末都开门营业.

- [throughout, adverb]  [出处](https://www.ldoceonline.com/dictionary/throughout)

    The debate continued, but Meredith remained silent `throughout`.

    译为: 辩论在继续, 但`在辩论整个期间`, Meredith 保持沉默.

    说明: 如果词性改为 preposition, 那么是 "throughout the debate".
